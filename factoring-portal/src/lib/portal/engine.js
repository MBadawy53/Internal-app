...
function caseActions(c){
  ...
  if(c.stage==='approved'){ if(r==='admin'||(typeof window!=='undefined'&&window.CF_BYPASS)) h+=btn('💸 Simulate Finance funding (SWIFT)','advanceCase(\''+c.id+'\',\'Funds disbursed via instant SWIFT (Finance system integration)\')','btn-gold'); else h+=info('var(--st-approved-bg)','var(--st-approved)','✓','Approved — Finance funds via the Finance system; status auto-syncs through the integration.'); return h; }
  if(c.stage==='funded'){ if(r==='admin'||(typeof window!=='undefined'&&window.CF_BYPASS)) h+=btn('Simulate settlement (proof of payment)','openSettle(\''+c.id+'\')','btn-gold'); else h+=info('var(--st-funded-bg)','var(--st-funded)','🏦','Funded — settlement handled in the Finance system; status auto-syncs through the integration.'); return h; }
  if(c.stage==='settled'){ if(r==='admin'||(typeof window!=='undefined'&&window.CF_BYPASS)) h+=btn('Close request','advanceCase(\''+c.id+'\',\'Request closed\')','btn-quiet'); else h+=info('var(--st-settled-bg)','var(--st-settled)','✓','Settled.'); return h; }
  return h;
}
...
function boot(){
  ...
  try{ if(/bypass/i.test((location.search||'')+(location.hash||''))) window.CF_BYPASS=true; }catch(e){}
  try{ const hp=(location.hash||'').replace('#','').replace(/[?&].*$/,''); if(hp==='employee'||hp==='client'){ authState.portal=hp; authState.tab=hp==='employee'?'ops':'client'; } }catch(e){}
  renderAuth();
}
